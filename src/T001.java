			...
			for(j = 0; j < 14; j++){
				if(i < 1) {
					if(j < 2) {
						...
						System.out.println("Insert into YZR.SEAT (THEATER_ID,PLEX_NUMBER,SEAT_TYPE,SEAT_INDEX,SEAT_NUMBER,RESERVATION_EXIST,START_TIME) values ('"+Tid+"','2','handicapped','" + index[i] +"','" + (j+1) + "','0','" + startTime + "');");
					} else if(j < 6) {
						System.out.println("Insert into YZR.SEAT (THEATER_ID,PLEX_NUMBER,SEAT_TYPE,SEAT_INDEX,SEAT_NUMBER,RESERVATION_EXIST,START_TIME) values ('"+Tid+"','2','empty','" + index[i] +"','" + (j+1) + "','0','" + startTime + "');");
					} else if(j < 13) {
